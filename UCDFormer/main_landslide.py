...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='Path/vgg16-397923af.pth')
    ...
    # prepare dataset
    dataPath_A = "Path/Style_Changes/landslide/pre.jpg"
    dataPath_B = "Path/Style_Changes/landslide/pos.jpg"
    ...
    for iteration in range(opts.epoch):
        for batch_num, image in enumerate((zip(image1, image2))):
            ...
            if step % 500 == 0:
                ...
                y_hat_image.save("Path/visualization/%d.png" % step)
                print("images saved")
            if step % 500 == 0:
                count = 0
                image_pr = []
                ...
                for i in range(len(ArrayReturn_pre)):
                    for j in range(len(ArrayReturn_pre[0])):
                        ...
                        # pre4 = pre4.data.cpu().numpy()
                        # pre4 = np.squeeze(pre4)
                        ...
                        # print("heatmap", detectedChangeMapNormalized.shape)
                        #cluster
                        ...
                        # print('Unchangelabel', Unchangelabel)
                        Changemap = np.where(fcm_label==Changelabel, 1, 0)
                        Uchangemap = np.where(fcm_label==Unchangelabel, 1, 0)
                        #change map2
                ...
                for i in range(binarymapm.shape[0]):
                    for j in range(binarymapm.shape[1]):
                        if binarymapm[i, j] == 1:
                            ...
                            # image_change[countc] = image_change[countc].reshape(-1, 1)
                        ...
                        if Unbinarymap[i,j] == 1:
                            ...
                            # image_unchange[countu] = image_unchange[countu].reshape(-1, 1)
                            label_unchange[countu] = np.zeros(1)
                            countu = countu + 1
                len_c = len(image_change)
                sample_list = range(len(image_unchange))
                samp_list = np.random.choice(sample_list, len_c, replace=False)
                samp_array = np.zeros(len(image_unchange))
                samp_array[samp_list] = 1
                image_un = np.array(image_unchange)
                label_un = np.array(label_unchange)
                image_unsample = image_un[np.where(samp_array == 1)]
                label_unsample = label_un[np.where(samp_array == 1)]
                image_unchan = list(image_unsample)
                label_unchan = list(label_unsample)
                images = image_change + image_unchan
                labels = label_change + label_unchan
                ...
                print("images saved")
                # accuracy obtained
                gt = Image.open("Path/Style_Changes/landslide/gt.png")
