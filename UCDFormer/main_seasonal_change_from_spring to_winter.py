...
if __name__ == '__main__':
    ...
    # prepare dataset
    dataPath_A = "Path/Seasonal_Changes/3_A.bmp"
    dataPath_B = "Path/Seasonal_Changes/3_B.bmp"
    ...
    # for i in range(0, len(ArrayReturn_pre) * len(ArrayReturn_pre[0])):
    #     image1.append([])
    #     image2.append([])
    ...
    #         image_pre = data_transform(image_pre)
    #         img1_tensor = torch.unsqueeze(image_pre, 0)
    #         image1[count] = img1_tensor
    #         image_pos = ArrayReturn_pos[i][j]
    #         image_pos = Image.fromarray(image_pos)
    #         img2_tensor = data_transform(image_pos)
    #         img2_tensor = torch.unsqueeze(img2_tensor, 0)
    #         image2[count] = img2_tensor
    #         count += 1
    # train network
    ...
    for iteration in range(opts.epoch):
        for batch_num, image in enumerate((zip(image1, image2))):
            ...
            if step % 500 == 0:
                ...
                for i in range(binarymapm.shape[0]):
                    for j in range(binarymapm.shape[1]):
                        if binarymapm[i, j] == 1:
                            ...
                            # img1_tensor = data_transform_test(img1)
                            # # img1_tensor = torch.unsqueeze(img1_tensor, 0)
                            # imge_pre_change[countc] = img1_tensor

                            # img2 = image_pos_change[countc]
                            # img2 = img2.transpose((1, 2, 0))
                            # img2 = Image.fromarray(img2)
                            # img2_tensor = data_transform_test(img2)
                            # # img2_tensor = torch.unsqueeze(img2_tensor, 0)
                            # image_pos_change[countc] = img2_tensor
                ...
                images = image_change + image_unchange[:len_c]
                labels = label_change + label_unchange[:len_c]
                ...
                # accuracy obtained
                gt = Image.open("Path/Seasonal_Changes/gt.png")
